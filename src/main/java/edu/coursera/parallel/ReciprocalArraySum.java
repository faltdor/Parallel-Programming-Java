...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        private static final int SEQUENTIAL_THRESHOLD = 500000;
        /**
        ...
        protected void compute() {
            if(endIndexExclusive - startIndexInclusive <= SEQUENTIAL_THRESHOLD){
                ...
                int middlePoint = (endIndexExclusive + startIndexInclusive) / 2;
                ReciprocalArraySumTask left = new ReciprocalArraySumTask(startIndexInclusive, middlePoint, input);
                ReciprocalArraySumTask right = new ReciprocalArraySumTask(middlePoint, endIndexExclusive, input);
    ...
    protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
        ...
        reciprocalArraySumTaskList.stream().skip(0).forEach(ReciprocalArraySumTask::fork);
        reciprocalArraySumTaskList.get(0).compute();
        reciprocalArraySumTaskList.stream().skip(0).forEach(ReciprocalArraySumTask::join);

        return reciprocalArraySumTaskList.stream().mapToDouble(ReciprocalArraySumTask::getValue).sum();
    }
}